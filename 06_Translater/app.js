...
btnTranslate.addEventListener("click",function () {
    ...
    var url = `https://api.mymemory.translated.net/get?q=${text}&langpair=${from}|${to}`;
...
// TODO :: look into other apis
// Todo :: Copy text and web speech api
for (let icon of icons) { // 2 tane icons sınıfına ait element var
    icon.addEventListener("click",(element) => {
        if (element.target.classList.contains("fa-copy")) {
            ...
            speechSynthesis.speak(utterance); // window. yazmaya gerek yok
        }
    })
}
